...
return jwt.sign(
{
id: user._id.toString(), email: user.email, name: user.name, },process.env.JWT_SECRET, { expiresIn: "7d" }
);
};
// Register new user
...
name, email, address, password: hashedPassword, about,
image: imageUrl, });
...
id: savedUser._id.toString(), name: savedUser.name, email: savedUser.email, address: savedUser.address,
...
id: user._id.toString(), name: user.name, email: user.email, address: user.address,
image: user.image, }, });
} catch (error) {
res.status(500).json({ message: error.message });
}
