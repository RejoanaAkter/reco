...
ingredients,
instructions, tags, cuisine, prepTime, cookingTime,
isPublic, } = req.body;
...
user: req.user._id, category, title, description,
ingredients: parsedIngredients,
instructions: parsedInstructions, tags: parsedTags, cuisine, prepTime, cookingTime,
imageUrl,
isPublic: isPublic === true || isPublic === "true" || isPublic === 1 || isPublic === "1", });
...
const { id } = req.params;
const {
title, description, category, cuisine, prepTime, cookingTime,
isPublic,
ingredients,
instructions, tags, } = req.body;
const parsedIngredients = safeJSONParse(ingredients, []);
const parsedInstructions = safeJSONParse(instructions, []);
const parsedTags = safeJSONParse(tags, []);
const updatedData = {
...
category: category || null, cuisine: cuisine?.trim() || "", prepTime: prepTime ? Number(prepTime) : 0, cookingTime: cookingTime ? Number(cookingTime) : 0,
isPublic: isPublic === true || isPublic === "true" || isPublic === 1 || isPublic === "1",
ingredients: parsedIngredients,
instructions: parsedInstructions, tags: parsedTags, };
if (req.file) {
const allowedTypes = ["image/jpeg", "image/jpg", "image/png"];
if (!allowedTypes.includes(req.file.mimetype)) {
return res.status(400).json({ message: "Only JPG or PNG images are allowed." });
...
res.status(500).json({ message: error.message || "Server error while updating recipe." });
}
};
...
recipes, total, page: Number(page), totalPages: Math.ceil(total / Number(limit)), });
} catch (err) {
...
await recipe.save();
res.status(200).json({
message: "Rating added/updated", ratings: recipe.ratings, });
} catch (err) {
console.error(err);
res.status(500).json({ message: err.message });
}
...
message: "Removed from favorites", favorites: recipe.favorites, });
} else {
...
res.status(201).json({
message: "Comment added", recipe: updatedRecipe, // Return full recipe for frontend
});
} catch (err) {
